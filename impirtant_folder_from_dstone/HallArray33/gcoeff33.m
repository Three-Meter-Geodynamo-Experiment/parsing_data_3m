function gauss = gcoeff33(debiasedhalldata,probepos33)
if ~(nargin==2)
	error('Usage: gausscoeff = gcoeff33(debiasedhalldata,probepos33)');
end
%% DO NOT USE, THIS ONE PROBABLY HAS SERIOUS PROBLEMS, USE gcoeff3m.m instead
probemask = ones(33,1);

%Title: gcoeff33
%Author: Matthew Adams
%Written: 16 Dec 2011 (adapted from getcoeff2011) 
...
% probe positions (copied from probepos2008_allprobes.dat, located on wave
% in /data/zero)
...
for j=1:nprobes
    if(probemask(j))
        r(i,1) = probepos33(j,1);
        theta(i,1) = probepos33(j,2);
        phi(i,1) = probepos33(j,3);
        i = i + 1;
    end
end
...
% ****
% disp(['nombre de coeffts HCN : ' int2str(ii)])
% *****
...
d = datafile;
% polar probes measure radial field with opposite orientation, so multiply
% by -1
d(:,32) = -1*d(:,32);
d(:,33) = -1*d(:,33);
