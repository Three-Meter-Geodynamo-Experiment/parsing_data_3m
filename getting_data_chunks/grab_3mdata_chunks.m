...
for file_i = 1:length(fileList)
    ...
    for i_t = 1:length(t1)
       if is_here(i_t)   % if the time chunk is in this file
           ...
           if t1(i_t) < time(1)
               disp(['t1=' num2str(t1(i_t)) ' t_index=' num2str(i_t) 't1 is less than the beginning of the file, so changing it to ' num2str(fix(time(1)))])
...
warning('Not everything was found')
return
